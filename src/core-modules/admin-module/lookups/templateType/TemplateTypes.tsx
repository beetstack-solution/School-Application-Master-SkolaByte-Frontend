...
import { deleteTemplateType, getTemplateTypes } from '@/api/admin-api/lookups-api/templateTypeApi';
...
function TemplateTypes() {
    ...
    const getAllTemplateType = async (page: number, limit: number) => {
        setLoading(true);

        try {
            const response: any = await getTemplateTypes(page, limit);
            if (response.success) {
                setFilteredTransportVehicles(response.data?.data);
                setTotalItems(response?.data?.total);
            } else {
                setError("Failed to fetch Template Type.");
            }
        } catch (error: any) {
    ...
    useEffect(() => {
        getAllTemplateType(currentPage, itemsPerPage);
    ...
    const handleStatusUpdate = async (_id: string, currentStatus: boolean) => {
        ...
        try {
            ...
            if (response?.success) {
                toast.success(
                    response.message || `Template Type status updated to ${newStatus ? "Active" : "Inactive"}.`
                );
                getAllTemplateType(currentPage, itemsPerPage);
            } else {
                toast.error(response.message || "Failed to update Template Type status.");
            }
        } catch (error: any) {
            toast.error(error.message || "Failed to update Template Type status.");
        }
    };
    ...
    const handleDelete = async (feeStructureId: string) => {
        const confirmDelete = window.confirm(
            "Are you sure you want to delete this Template Type?"
        );
        if (!confirmDelete) return;

        try {
            const response: any = await deleteTemplateType(feeStructureId);
            if (response.success) {
                toast.success(response.message || "Template Type deleted successfully!");
                getAllTemplateType(currentPage, itemsPerPage);
            } else {
                toast.error(response.message || "Failed to delete Template Type.");
            }
        } catch (error: any) {
            toast.error(error.message || "Failed to delete Template Type.");
        }
    };
       ...
       />
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-2">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800">Template Type</h2>
                        <div className="mt-1">
                ...
                {showModal && (
                    <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
                        <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                            <AddTemplateType onClose={handleClose} onReload={handleReload} />
                        </div>
                    </div>
                )}
