...
const EditPtaRole: React.FC<AddFeeInstallmentTypeProps> = ({ onClose, onReload, ptaRoleId }) => {
    const [type, setType] = useState<any>('');
    const [error, setError] = useState<string>('');
    const [isSubmitting, setIsSubmitting] = useState(false);

    const fetchPtaRoleById = async (id: string) => {
        try {
            const response: any = await getPtaRoleById(id);
            if (response.success) {
                setType(response.data.name);
            } else {
                console.error('Error in API response:', response.message);
                toast.error(response.message || 'Failed to fetch PTA Role by ID.');
            }
        } catch (error: any) {
            console.error('Error fetching PTA Role by ID:', error);
            toast.error('Failed to fetch PTA Role by ID. Please try again.');
        }
    };

   useEffect(() => {
       if (ptaRoleId){
            fetchPtaRoleById(ptaRoleId);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response: any = await updatePtaRole(ptaRoleId, payload);
            if (response.success) {
