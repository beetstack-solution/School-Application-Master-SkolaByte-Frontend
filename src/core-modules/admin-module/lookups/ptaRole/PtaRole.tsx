...
function PtaRole() {
    ...
    const getAllPtaRoles = async (page: number, limit: number) => {
        ...
        try {
            const response: any = await getPtaRoles(page, limit);
            if (response.success) {
                setFilteredTransportVehicles(response.data?.data);
                setTotalItems(response?.data?.total);
            } else {
                setError("Failed to fetch PTA Role.");
            }
        } catch (error: any) {
    ...
    const handleStatusUpdate = async (_id: string, currentStatus: boolean) => {
        ...
        try {
            ...
            if (response?.success) {
                toast.success(
                    response.message || `PTA Role status updated to ${newStatus ? "Active" : "Inactive"}.`
                );
                getAllPtaRoles(currentPage, itemsPerPage);
            } else {
                toast.error(response.message || "Failed to update PTA Role status.");
            }
        } catch (error: any) {
            toast.error(error.message || "Failed to update PTA Role status.");
        }
    };
    ...
    const handleDelete = async (feeStructureId: string) => {
        const confirmDelete = window.confirm(
            "Are you sure you want to delete this PTA Role?"
        );
        ...
        try {
            ...
            if (response.success) {
                toast.success(response.message || "PTA Role deleted successfully!");
                getAllPtaRoles(currentPage, itemsPerPage);
            } else {
                toast.error(response.message || "Failed to delete PTA Role.");
            }
        } catch (error: any) {
            toast.error(error.message || "Failed to delete PTA Role.");
        }
    };
       ...
       />
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-2">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800">PTA Roles</h2>
                        <div className="mt-1">
