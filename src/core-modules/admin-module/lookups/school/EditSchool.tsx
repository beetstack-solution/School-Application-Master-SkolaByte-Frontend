...
import { getSchoolProfileById, updateSchoolProfileById } from '@/api/admin-api/lookups-api/schoolApi';
function EditSchool() {
    const { id } = useParams();
        ...
        branchDetails: [
            {
                name: '',
                code: '',
                address: [  // Now an array of addresses
                    {
                        street: '',
                        city: '',
                        state: '',
...
const fetchSchoolDataById = async (id: string) => {
    try {
        ...
        setFormData({
            ...
            existingImages: schoolData.imageUrl || [],
            files: [],
            onBoarded: schoolData.onBoarded,
            address: schoolData.address,
            contact: schoolData.contact,
            facilities: schoolData.facilities,
            branchDetails: schoolData.branchDetails.map((branch: any) => ({
                name: branch.name,
                code: branch.code,
                address: branch.address.map((addr: any) => ({
                    street: addr.street,
                    city: addr.city,
                    state: addr.state,
        ...
        if (name.includes('.')) {
            ...
            if (parts[0] === 'branchDetails') {
                ...
                setFormData(prev => {
                    ...
                    if (field === 'address') {
                        // Handle branchDetails[0].address[0].street etc.
                        const addrIndex = parseInt(parts[3]);
                        const subfield = parts[4];

                        newBranchDetails[index] = {
                            ...newBranchDetails[index],
                            address: newBranchDetails[index].address.map((addr, i) =>
                                i === addrIndex ? { ...addr, [subfield]: value } : addr
                            )
                        };
                    } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            formData.branchDetails.forEach((branch: any, index: number) => {
                ...
                // Append each address in the branch's address array
                branch.address.forEach((addr: any, addrIndex: number) => {
                    formDataToSend.append(`branchDetails[${index}][address][${addrIndex}][street]`, addr.street);
                    formDataToSend.append(`branchDetails[${index}][address][${addrIndex}][city]`, addr.city);
                    formDataToSend.append(`branchDetails[${index}][address][${addrIndex}][state]`, addr.state);
                    formDataToSend.append(`branchDetails[${index}][address][${addrIndex}][country]`, addr.country);
                    formDataToSend.append(`branchDetails[${index}][address][${addrIndex}][postalCode]`, addr.postalCode);
                });
            });
            ...
            // Send the form data
            const response = await updateSchoolProfileById(id as string, formDataToSend);
                    ...
                    <div className="space-y-4 md:col-span-2">
                        ...
                        {formData.branchDetails && formData.branchDetails.map((branch: any, index) => (
                            <div key={index} className="border p-4 rounded-lg space-y-4">
                                ...
                                {branch.address.map((addr: any, addrIndex: number) => (
                                    <div key={addrIndex} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Street</label>
                                            <input
                                                type="text"
                                                name={`branchDetails.${index}.address.${addrIndex}.street`}
                                                value={addr.street}
                                                onChange={handleChange}
                                                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                                                required
                                        ...
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                name={`branchDetails.${index}.address.${addrIndex}.city`}
                                                value={addr.city}
                                                onChange={handleChange}
                                                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                                                required
                                        ...
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                name={`branchDetails.${index}.address.${addrIndex}.state`}
                                                value={addr.state}
                                                onChange={handleChange}
                                                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                                                required
                                        ...
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                name={`branchDetails.${index}.address.${addrIndex}.country`}
                                                value={addr.country}
                                                onChange={handleChange}
                                                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                                                required
                                        ...
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                name={`branchDetails.${index}.address.${addrIndex}.postalCode`}
